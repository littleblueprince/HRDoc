...
class Left_Brace:
    ...
    """
    def __init__(self, position, depth, right_brace_position=-1):
        self.position = position
        self.depth = depth # logging the nest depth of {}
        self.right_brace_position = right_brace_position
    def set_right_brace(self, right_brace_position): # * logging crosponding closed } position
        assert right_brace_position > self.position
        self.right_brace_position = right_brace_position
    def get_braced_content(self, content): # * getting the content closed by this Left_Brace node
        assert max(self.right_brace_position, self.position) < len(content)
        assert min(self.right_brace_position, self.position) > -1
        assert self.right_brace_position > self.position
        return content[self.position+1 : self.right_brace_position]
    def is_continous_brace(self, right_node): # * verify 
        assert isinstance(right_node, Left_Brace)
        return right_node.position == self.right_brace_position + 1
