...
def transfer_json(pdf_path, ori_jd):
    tgt_json = {"title": [], "author": [], "affili": [], "mail": [], "sec": [], "fig": [], "tab": [], "foot": [],
                "fnote": []}
    ...
    for line in ori_jd:
        ...
        if line["class"] in meta:
            tgt_json[line["class"]].append(
                {"text": line["text"], "box": line["box"], "line_id": line["line_id"], "page": line["page"]})
            line["visited"] = True
    for key in meta:
    ...
    for key in fig_tab:  # find all caption
        for item in tgt_json[key]:
            ...
            assert len(connect_ids) == 0, "Find {} has connent node in {}, line id: {}".format(key, pdf_path,
                                                                                               item["line_id"])
            if len(child_ids) != 0:
                item["caption"] = []
                for child_id in child_ids:
                    ...
                    assert len(sub_child_ids) == 0, "Find {}'s caption has child in {}, line id: {}".format(key,
                                                                                                            pdf_path,
                                                                                                            item[
                                                                                                                "line_id"])
                    if len(sub_connect_ids) != 0:
                        ...
                        for sub_connect_id in sub_connect_ids:
                            ...
                            cur_caption["connect"].append(
                                {"text": line["text"], "box": line["box"], "line_id": line["line_id"],
                                 "page": line["page"]})
                            line["visited"] = True
    # Renew fstline
    prev_fstline = -1
    ...
    for key in fig_tab:
        ...
        for item in final_json[key]:
            ...
            if "caption" in item.keys():
                assert len(item["caption"]) == 1, "Find {} has more than one caption in {}, line id: {}".format(key,
                                                                                                                file_path,
                                                                                                                item[
                                                                                                                    "line_id"])
...
def parser_args():
    argparser = argparse.ArgumentParser(
        description="Transfer json that contain information for each line to json that hierarchically store document information")
